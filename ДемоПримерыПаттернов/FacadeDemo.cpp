/*
Этот пример показывает структуру паттерна Фасад — из каких классов он состоит, какие роли эти классы выполняют
...
* Класс Фасада предоставляет простой интерфейс для сложной логики одной или нескольких подсистем. 
  Фасад:
  - делегирует запросы клиентов соответствующим объектам внутри подсистемы,
  - отвечает за управление их жизненным циклом. 
...
protected:
	...
	/**
	* В зависимости от потребностей приложения вы можете предоставить
...
public:
	...
	{
		(subsystem1 == nullptr) ? this->subsystem1_ = new Subsystem1 : this->subsystem1_ = subsystem1;
		(subsystem1 == nullptr) ? this->subsystem2_ = new Subsystem2 : this->subsystem2_ = subsystem2;
	}
	~Facade() 
	{
		delete subsystem1_;
...
* В клиентском коде могут быть уже созданы некоторые объекты подсистемы. 
  В этом случае может оказаться целесообразным инициализировать Фасад с этими
